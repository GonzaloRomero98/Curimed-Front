...
export class ReservasComponent implements OnInit {
  ...
  medicos: Medico[] = [];
  servicios: Servicio[] = [];
  fechaHabil = (fecha: Date | null) => !!fecha && fecha.getDay() !== 0;
  ...
  async buscarPaciente(): Promise<void> {
    const rutCtrl = this.form.get('rut');
    if (!rutCtrl || !rutCtrl.value) {
      rutCtrl?.markAsTouched();
      this.snackBar.open('Debe ingresar un RUT para buscar', 'Cerrar', { duration: 2000 });
      return;
    }

    this.cargarPaciente = true;
    try {
      const resp = await firstValueFrom(this.api.buscarPaciente(rutCtrl.value));
      console.log(resp);
      const paciente: Paciente | null = Array.isArray(resp) ? (resp[0] ?? null) : (resp ?? null);
      if (!paciente) {
        this.snackBar.open('No se encontró un paciente con el RUT ingresado', 'Cerrar', { duration: 2000 });
        this.form.patchValue({ nombres: '', apellidos: '', celular: '', correo: '' });
        return;
      }
      this.form.patchValue({
        ...
        correo: paciente.direccion,
      });
    } catch (e) {
        console.error(e);
        this.snackBar.open('Error al buscar el paciente, intente nuevamente', 'Cerrar', { duration: 2000 });
    } finally {
  ...
  try {
    ...
    const parsed: Slot[] = raw.map((s: any) => {
      ...
      return {
        inicio: start,
        fin: end,
        disponible: !s.desabilitado, // (respeta el nombre que te manda el backend)
      };
    })
