...
import { BuscarPacienteService, Comuna, PacineteCreate } from "../../../../core/services/buscarPaciente.service";
...
function validarRut(control:AbstractControl):ValidationErrors | null{
    const val = (control.value ?? '').toString().toUpperCase().replace(/[.\-]/g, '');
    if(!val)return null;
    if(!/^\d{7,8}[0-9K]$/.test(val)) return {rut:'Formato Invalido'};
...
export class Registro implements OnInit{
    ...
    private humnizarErrores(err:any, paso:'usuario'|'paciente'):string{
    ...
    async submit(){
        ...
        try{
            ...
            const msg = this.humnizarErrores(e, paso);
            this.snack.open(`❌ ${msg}`, 'OK', { duration: 5000 });

            if(paso === 'paciente' && usuario_id){
                try { 
                    await firstValueFrom(this.api.eliminarUsuario(usuario_id)); 
                } catch {}
            }
        }finally{
